...
export default async (req, res) => {
  ...
  const logUrl = "logs/rest/storage.json";
  ...
  switch (method) {
    ...
    case 'GET':
      if (query?.userId) {
        res.status(200).json({ name: 'REST Logs File', file: file.filter(item => +item.userId === +query.userId)});
      } else {
        res.status(200).json({ name: 'REST Logs File', file});
      }
      break;
    default:
      break;
